...
Function generate_codebook() adapted for returns multiple codebook sizes
By Rafael Feitosa
"""
...
def generate_codebook(data, size_codebook, epsilon):
    ...
    # adapted
    codebooks = dict()
    if type(size_codebook) == int: # if is only one codebook size
        ...
        codebooks[size_codebook] = codebook, avg_dist
        
        return codebooks
    else: # if is a list of codebooks sizes
        # split codevectors until we have have enough
        while len(codebook) < max(size_codebook):
            codebook, codebook_abs_weights, codebook_rel_weights, avg_dist = split_codebook(data, codebook, epsilon, avg_dist)
            
            # adapted                                                                              
            if len(codebook) in size_codebook:
                codebooks[len(codebook)] = codebook, avg_dist
    
        # adapted
        return codebooks
        #return codebook, codebook_abs_weights, codebook_rel_weights, avg_dist
